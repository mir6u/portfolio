  ...
  return (
    <li className={`relative mb-4 ${extraClasses}`}>
      <div ref={myRefT} className="">
        <div
          ref={myRefT1}
          className="w-3 pcard h-3 z-20 bg-gradient-to-r from-cyan-400 to-green-300 rounded-full absolute scale-120  sm:mt-0 left-[calc(50%-6px)] animate-ping"
        ></div>
        <div
          ref={myRefT2}
          className="w-3 pcard h-3 z-29 bg-gradient-to-r from-cyan-300 to-green-300 rounded-full absolute scale-120  left-[calc(50%-6px)] "
        ></div>
        {isNotLast ? <TimeLineLine /> : null}
      </div>
